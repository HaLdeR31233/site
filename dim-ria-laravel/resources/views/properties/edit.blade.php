...
@section('title', 'Редагувати нерухомість - DIM.RIA')
...
<div class="container">
    ...
    <form action="{{ route('properties.update', $property) }}" method="POST">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select @error('type') is-invalid @enderror" 
                    ...
                    <option value="apartment" {{ old('type', $property->type) === 'apartment' ? 'selected' : '' }}>Квартира</option>
                    <option value="house" {{ old('type', $property->type) === 'house' ? 'selected' : '' }}>Будинок</option>
                    <option value="commercial" {{ old('type', $property->type) === 'commercial' ? 'selected' : '' }}>Комерційна</option>
                </select>
                @error('type')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                ...
                <input type="number" class="form-control @error('rooms') is-invalid @enderror" 
                       id="rooms" name="rooms" value="{{ old('rooms', $property->rooms) }}" min="1" required>
                @error('rooms')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" 
                    ...
                    <option value="active" {{ old('status', $property->status) === 'active' ? 'selected' : '' }}>Активна</option>
                    <option value="sold" {{ old('status', $property->status) === 'sold' ? 'selected' : '' }}>Продана</option>
                    <option value="rented" {{ old('status', $property->status) === 'rented' ? 'selected' : '' }}>Орендована</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('properties.show', $property) }}" class="btn btn-secondary">Скасувати</a>
        </div>
    </form>
</div>
@endsection
